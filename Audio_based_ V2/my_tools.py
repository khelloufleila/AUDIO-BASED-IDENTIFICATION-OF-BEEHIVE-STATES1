...
def frames(a,fsize,hopsize,nframes=0):
	...
	if (nframes == 0) or (nframes > max_nf):
		nframes	= max_nf; 
	b 		= numpy.ones((int(fsize), int(nframes)), int);	
...
def fbankpwrsmooth(a,Fs,cfarray):
	...
	for j in range(0, nchans):
		b[j,:]	= a[j,:]+ numpy.concatenate( (a[j,int(shift[j]):], numpy.zeros(int(shift[j]), float))) / 2.;
	return b;
