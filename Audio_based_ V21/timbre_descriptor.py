...
def check_values(param_val, param_ref, field_name, Thr=0.1):
	...
	err = numpy.abs(param_ref - param_val);
...
## test script 1: compare descriptor values with a reference file
def test_descriptor():
	#filename = '01 Just One Of Those Things-19.wav';	
	...
	tic = time.time();
	desc_TEE, desc_AS, desc_STFTmag, desc_STFTpow, desc_Har, desc_ERB, desc_GAM = compute_all_descriptor(s, Fs);
	...
	#m = scipy.io.loadmat('param_ref.mat');
	#param_ref = numpy.squeeze(numpy.array(m['param_ref']));
	#check_values(param_val, param_ref, field_name);
		
	## for debug only
	toc = time.time()-tic;
	print("Execution time (in sec):", toc, "\n\n");
	return s, Fs, desc, param_val, field_name;

## test script 2: compare descriptor values with a reference file
def test_descriptor2():
	#filename = '01 Just One Of Those Things-19.wav';	
	#Fs, s = wavfile.read(filename);
	#s = s/MAX_VAL;
	m = scipy.io.loadmat('test_desc.mat');
	s = m['s'];
	Fs = m['Fs'][0];
	s = sum(s.T) / 2;
	N = len(s);
	...
	desc_struc, t = timbre_desc_time(s, Fs, int(0.5 * Fs), 2);
	
	## for debug only
	toc = time.time()-tic;
	print("Execution time (in sec):", toc, "\n\n");
	return s, Fs, desc_struc, t;
...
def temporalmodeling(desc):
	...
	for i in range(0, len(desc)):
		for j in range(0, len(desc[i]._fields)):
			...
			if i == 0 and j < len(desc[i]._fields)-1:   
				...
				if desc[i]._fields[j] == "AutoCorr":
					...
					for k in range(0, config_s.xcorr_nb_coeff):
...
def compute_all_descriptor(trame_s, Fs):
	...
	if Fs != 44100:
		print("!! WARNING, Resampling from ", Fs, " to Fs=44100 Hz (poor quality expected !!");
		trame_s = scipy.signal.resample(trame_s, int(len(trame_s)/Fs * 44100));
		Fs = 44100
...
def ERBRep(f_Sig_v, Fs):
	...
	#loud_erb			= numpy.sum(S_erb);
	#troids_erb			= mt.centroid(S_erb);
	#troid_erb			= numpy.sum(troids_erb * loud_erb)/ numpy.sum(loud_erb);	# weighted average
	#centroid_erb		= mt.my_interpolate(numpy.arange(0,nchans), f, troid_erb);		# to hz
	#centroids_erb 		= mt.my_interpolate(numpy.arange(0,nchans), f, troids_erb);		# to hz
	...
	#loud_gam			= numpy.sum(S_gam);
	#troids_gam			= mt.centroid(S_gam);
	#troid_gam			= numpy.sum(troids_gam * loud_gam) / numpy.sum(loud_gam);		# weighted average
	#centroid_gam		= mt.my_interpolate(numpy.arange(0,nchans), f, troid_gam);		# to hz
	#centroids_gam 		= mt.my_interpolate(numpy.arange(0,nchans), f, troids_gam);		# to hz
	i_SizeX2    = len(t_gam);
	i_SizeY2    = len(f_gam);
	f_SupX_v2	= t_gam;
	f_SupY_v2	= f_gam/Fs;
...
def FHarrep(f_Sig_v, Fs):
	...
	#p_v	= numpy.array(m['p_v'])[:,0];
	#t_v	= numpy.array(m['t_v'])[:,0];
	#s_v	= numpy.array(m['s_v'])[:,0];
	...
	fenetre_v		= numpy.ones(int(L_n), float); #scipy.signal.boxcar(L_n);
	...
	for num_inharmo in range(0, nb_inharmo):
		...
		for num_delta in range(0, nb_delta):
			...
			for num_frame in range(0, nb_frame):
...
def FCalcDescr( f_DistrPts_m, i_SizeX, i_SizeY, f_SupX_v, f_SupY_v ):
	...
	f_SpecDecr_v	= numpy.dot(f_Den_v, f_Num_m) /  numpy.sum(f_DistrPts_m+EPS, axis=0); #[1:i_SizeY,:]
	
	#print  "chatt: ", numpy.shape(f_Num_m), " - ", numpy.shape(f_Den_v)
	#print "SUM:", numpy.sum(f_DistrPts_m[1:i_SizeY, :])
	#print "SUM:", numpy.sum(numpy.repeat([f_DistrPts_m[0,:],], i_SizeY-1, 0))	
	#my_plot(f_DistrPts_m[0,:])
...
def TCalcDescr(trame_s, Fs):
	...
	RMSEnv		= f_Env_v);			# === GFP 2010/11/16
	#ADSR_v		= f_ADSR_v[0 1 4];		# === attack-decay-sustain-release envelope
